...
export async function GET(request: NextRequest) {
  try {
    if (!process.env.DATABASE_URL) {
      return NextResponse.json({ jobs: [], page: 1, pageSize: 10, total: 0, totalPages: 0, storage: "not_configured" });
    }
    ...
    if (!user) {
      return NextResponse.json({ jobs: [], page, pageSize, total: 0, totalPages: 0, storage: "empty" });
    }
    ...
    return NextResponse.json({ jobs: [], page: 1, pageSize: 10, total: 0, totalPages: 0, storage: "unavailable", error: error instanceof Error ? error.message : "Saved jobs could not be loaded." });
  }
}
